...
//矩阵的接口pin   一定要记得改！！！！！！！！！！
//此处是矩阵棋盘的PIN，一定要改！！！！！！！！
const int rowPins[length_x] = { 24,26,50,31,33,49,52,53,51,41};
const int colPins[length_y] = { 30,34,37,17,23,27,57,16,25,29};
//此处是共用的步进电机的引脚
//此处是X，Y轴的步进电机的引脚
...
const int Step_Motor_Z_Distance = 100;//定义Z轴步进电机运动的距离
const int percent_x = 4;
const int percent_y = 8;
//此处定义的是一个格子的大小，即需要步进电机转多少圈
const int Square_Size_X = 1;		//8.125*200*percent_x;//32.5
const int Square_Size_Y = 1;		//0.8125*200*percent_y;//32.5
//步进电机的防抖动时间设定
const int delayms_low = 1;	//400;
const int delayms_high = 1;	//2;
//复位点到棋盒
const long Distance_X = 1;//		132.5 * 200 * percent_x;
const long Distance_Y = 1;//		5 * 200 * percent_y;//13.25
//此处定义的是从棋盒到（0，0）坐标需要的距离
const long Distance_X_ZERO = 1;	// 97.5 * 200 * percent_x;//39cm
const long Distance_Y_ZERO = 1;	// 1.625 * 200 * percent_y;//6.5cm
//X和Y和Z轴的复位限位开关引脚
...
private:
	...
	int latest_x;//最近一次落子X点坐标
	int latest_y;//最近一次落子Y点坐标
	int over;//是否结束，涉及到不重启的重新开始事件
	//================
	//请在覆盖最近落子坐标前，执行胜负判断
	//================
	//等待算法实现，算法返回机器落子的X和Y点，覆盖原有的最近一次落子坐标
	//假定使用DFS和BFS
